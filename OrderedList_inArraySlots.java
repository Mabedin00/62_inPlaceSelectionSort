...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
        ...
        for (int idx = 0; idx < unordered.size(); idx ++) {
             int nextLargerAt = champIndex(idx);
             int champ = unordered.get(nextLargerAt);
             int temp = unordered.get(idx);
             unordered.set(nextLargerAt, temp);
             unordered.set(idx, champ);
     ...
     */
     private int champIndex(int start) {
        ...
        for (int idx = start; idx < list_iAS.size(); idx++){
            Integer el = list_iAS.get(idx);
            if (el != null && el < champ){
                champ = el;
                // System.out.println(champ);
            }
        }
        return list_iAS.indexOf(champ);
     }
